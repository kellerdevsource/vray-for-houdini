...
int VRayProxyROP::getExportOptions(fpreal time, VRayProxyExportOptions &options) const
{
	...
	options.m_maxPreviewFaces    = std::max(static_cast<int>(evalInt("max_previewfaces", 0, time)), 0);
	options.m_maxPreviewStrands  = std::max(static_cast<int>(evalInt("max_previewstrands", 0, time)), 0);
	options.m_maxFacesPerVoxel   = (evalInt("voxelpermesh", 0, time))? 0 : std::max(static_cast<int>(evalInt("max_facespervoxel", 0, time)), 0);
