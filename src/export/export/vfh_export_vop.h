...
struct ShaderExporter
{
	...
	VRay::PluginRef exportShaderNode(const OP_Node &opNode);
	...
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportConnectedSocket(const VOP_Node &vopNode, const UT_String &socketName);

	/// Export plugin from node connected to the socket.
	/// @param vopNode Currently processed node.
	/// @param socketName Currently processed socket name.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportConnectedSocket(const VOP_Node &vopNode, const QString &socketName);

	/// Export plugin from node connected to the socket.
	/// @param vopNode Currently processed node.
	/// @param socketIndex Currently processed socket index.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportConnectedSocket(const VOP_Node &vopNode, int socketIndex);

	/// Export plugin from node connected to the socket.
	/// @param vopNode Currently processed node.
	/// @param fromSocket Currently processed socket info.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportConnectedSocket(const VOP_Node &vopNode, const SocketInfo &fromSocket);

	/// Export plugins from all connected sockets.
	/// @param vopNode Currently processed node.
	/// @param pluginDesc Plugin description for @a vopNode.
	void exportConnectedSockets(const VOP_Node &vopNode, Attrs::PluginDesc &pluginDesc);
...
private:
	...
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportNull(const VOP_Node &null, const SocketInfo &fromSocket);

	/// Export plugin from "switcher" node.
	/// @param switcher "switcher" node.
	/// @param fromSocket Where this node is connected to.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportSwitcher(const VOP_Node &switcher, const SocketInfo &fromSocket);

	/// Export plugin from "subnet" node.
	/// @param subnet "subnet" node.
	/// @param fromSocket Where this node is connected to.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportSubnet(const VOP_Node &subnet, const SocketInfo &fromSocket);

	/// Export plugin from "subinput" node.
	/// @param subnetInput "subinput" node.
	/// @param fromSocket Where this node is connected to.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportSubnetInput(const VOP_Node &subnetInput, const SocketInfo &fromSocket);

	/// Export "V-Ray Material Buider" SHOP node.
	/// @param shopNode Currently processed node.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportVRayMaterialBuilder(const SHOP_Node &shopNode);

	/// Export "V-Ray Material Output" node.
	/// @param vopNode Currently processed node.
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportVRayMaterialOutput(const VOP_Node &vopNode);

	/// Export custom V-Ray VOP node.
	/// @param vopNode Currently processed node.
	/// @param fromSocket Where this node is connected to. 
	/// @returns VRay::PluginRef. May be empty for unsupported types or any exporting error.
	VRay::PluginRef exportVRayNode(const VOP_Node &vopNode, const SocketInfo &fromSocket);
